...
/**
 * Class that handles the RCON part of NBOT.
 */
class NBotBERcon
{
  /**
   ...
   * @param {NBot} nbot
   */
  constructor(data, beRconData, bot, nbot) {
    /**
     * @type {JSON}
     */
  ...
  init() {
    ...
    if(this._continue) {
      ...
      this._nbot.logBot('Login ' + this._guild.name + ' : ' + this._defaultChannel.name);
      this.initEvents();
      if(this._bnode) {
        this._bnode.login();
      }
  ...
  initEvents() {
    ...
    this._bnode.on('disconnected', (extra) => {
      this.cancelCronJobs();
      if(extra && extra == 'stop') {
        console.log('RCON stopped on purpose. NBot reload for ' + this._guild.name);
        this._nbot.logBot('RCON stopped on purpose. NBot reload for ' + this._guild.name);
        if (this._commandChannel) {
          this.sendTextMessage(this._commandChannel, 'RCON server disconnected on purpose. NBot reload');
        }
        ...
        this._nbot.logBot('RCON server disconnected. Trying to reconnect in ' + (this._data.config.rconReconnect / 1000 ) + ' sec for ' + this._guild.name);
        if (this._commandChannel) {
        ...
        let wait = new Promise((resolve, reject) => {
          ...
          this._nbot.logBot('Creating new rcon for ' + this._guild.name);
  ...
  loginAction(err, success) {
    if (err) {
      ...
      this._nbot.logBot('Unable to connect to server for ' + this._guild.name);
      if (this._commandChannel) {
      ...
      this._nbot.logBot('LoginAttempts ' + this._loginAttempts);
      if(this._loginAttempts < 60) {
        ...
        this._nbot.logBot('Failed ' + this._loginAttempts + ' times to connect to the gameserver of ' + this._guild.name );
      }
    } else {
      if (success == true) {
        console.log('Logged in RCON successfully for ' + this._guild.name);
        this._nbot.logBot('Logged in RCON successfully for ' + this._guild.name);
        ...
        this._nbot.logBot('RCON login failed! (password may be incorrect) for ' + this._guild.name);
        if (this._commandChannel) {
          this.sendTextMessage( this._commandChannel, 'RCON login failed! Check your authentications and nbot reload.');
        }
      }
    }
  }
  ...
  listCommands() {
    ...
    let fullMessage = '# NBot BattlEye RCON commands\n' +
      '\n' +
  ...
  cmdSteamTestPrintData(message) {
    ...
    Gamedig.query(
      ...
      (state) => {
        if(state.error) {
          console.log("Server is offline");
          this._nbot.logBot(JSON.stringify(state, null, 2));
          console.log(state);
        } else {
          console.log(state);
          // this._nbot.logBot(JSON.stringify(state, null, 2));
