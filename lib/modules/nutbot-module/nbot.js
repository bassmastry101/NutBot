...
class NBot
{
  ...
  constructor(options) {
    ...
    console.log('NBot Module ready');
  }

  /**
   * Gets the rcon instances of nbot
  ...
  /**
   * Returns a list of active guilds used by nbot
   * @returns {Array<Guild>}
   */
  get activeGuilds() {
  ...
  findAllActiveGuilds() {
    ...
    this.bot.guilds.array().forEach((element) => {
      ...
      this._data[String(element.id)] = {
        ...
        'beRcons': [],
        'channels': { // Config for nbot channels - (rcon channels are the rcons object since you can have multiple rcons ervers.)
          'welcome': 'welcome',
          'goodbye': 'welcome'
        }
      };
  ...
  /**
   * Binds the nbot channel for loggin.
   */
  setLogChannel() {
    let guilds = this.bot.guilds;
    if (guilds) {
      let logguild = guilds.find('name','NBot');
      if(logguild) {
        this._logChannel = logguild.channels.find('name', 'nlog');
      }
    }
  }
  ...
  broadcastMessage(text) {
    /**
     * @var {NBotBERcon} bercon
     */
    this._bercons.forEach((bercon) => {
      this.sendTextMessage(bercon.commandChannel, text.replace('!nbot broadcast ',''));
    });
  }
   ...
   */
  listStats(channel) {
    this.sendTextMessage(channel,`NutBot is serving in ${this.bot.channels.size} channels on ${this.bot.guilds.size} servers, for a total of ${this.bot.users.size} users.`);
    this.sendTextMessage(channel,`This includes a total of ${this._bercons.length} instances.`);
  }
